...
* @version 4.2.3.1
*/error_reporting(6135);$Kc=!preg_match('~^(unsafe_raw)?$~',ini_get("filter.default"));if($Kc||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$X){$Ih=filter_input_array(constant("INPUT$X"),FILTER_UNSAFE_RAW);if($Ih)$$X=$Ih;}}if(function_exists("mb_internal_encoding"))mb_internal_encoding("8bit");if(isset($_GET["file"])){if($_SERVER["HTTP_IF_MODIFIED_SINCE"]){header("HTTP/1.1 304 Not Modified");exit;}header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");header("Last-Modified: ".gmdate("D, d M Y H:i:s")." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
bracket_escape($t,$Na=false){static$vh=array(':'=>':1',']'=>':2','['=>':3');return
strtr($t,($Na?array_flip($vh):$vh));}function
...
optionlist($We,$ug=null,$Qh=false){$J="";foreach($We
as$Hd=>$W){$Xe=array($Hd=>$W);if(is_array($W)){$J.='<optgroup label="'.h($Hd).'">';$Xe=$W;}foreach($Xe
as$x=>$X)$J.='<option'.($Qh||is_string($x)?' value="'.h($x).'"':'').(($Qh||is_string($x)?(string)$x:$X)===$ug?' selected':'').'>'.h($X);if(is_array($W))$J.='</optgroup>';}return$J;}function
html_select($C,$We,$Y="",$Pe=true){if($Pe)return"<select name='".h($C)."'".(is_string($Pe)?' onchange="'.h($Pe).'"':"").">".optionlist($We,$Y)."</select>";$J="";foreach($We
as$x=>$X)$J.="<label><input type='radio' name='".h($C)."' value='".h($x)."'".($x==$Y?" checked":"").">".h($X)."</label>";return$J;}function
select_input($Ja,$We,$Y="",$wf=""){return($We?"<select$Ja><option value=''>$wf".optionlist($We,$Y,true)."</select>":"<input$Ja size='10' value='".h($Y)."' placeholder='$wf'>");}function
confirm(){return" onclick=\"return confirm('".lang(0)."');\"";}function
print_fieldset($kd,$Wd,$bi=false,$Qe=""){echo"<fieldset><legend><a href='#fieldset-$kd' onclick=\"".h($Qe)."return !toggle('fieldset-$kd');\">$Wd</a></legend><div id='fieldset-$kd'".($bi?"":" class='hidden'").">\n";}function
...
set_password($Xh,$N,$V,$G){$_SESSION["pwds"][$Xh][$N][$V]=($_COOKIE["adminer_key"]&&is_string($G)?array(encrypt_string($G,$_COOKIE["adminer_key"])):$G);}function
...
get_key_vals($H,$i=null,$lh=0){global$h;if(!is_object($i))$i=$h;$J=array();$i->timeout=$lh;$I=$i->query($H);$i->timeout=0;if(is_object($I)){while($K=$I->fetch_row())$J[$K[0]]=$K[1];}return$J;}function
...
auth_url($Xh,$N,$V,$m=null){global$Yb;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($Yb))."|username|".($m!==null?"db|":"").session_name()),$B);return"$B[1]?".(sid()?SID."&":"").($Xh!="server"||$N!=""?urlencode($Xh)."=".urlencode($N)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($B[2]?"&$B[2]":"");}function
...
query_redirect($H,$A,$ne,$Tf=true,$xc=true,$Ec=false,$kh=""){global$h,$n,$b;if($xc){$Jg=microtime(true);$Ec=!$h->query($H);$kh=format_time($Jg);}$Hg="";if($H)$Hg=$b->messageQuery($H,$kh);if($Ec){$n=error().$Hg;return
false;}if($Tf)redirect($A,$ne.$Hg);return
true;}function
queries($H){global$h;static$Nf=array();static$Jg;if(!$Jg)$Jg=microtime(true);if($H===null)return
array(implode("\n",$Nf),format_time($Jg));$Nf[]=(preg_match('~;$~',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H).";";return$h->query($H);}function
apply_queries($H,$S,$tc='table'){foreach($S
as$Q){if(!queries("$H ".$tc($Q)))return
false;}return
true;}function
queries_redirect($A,$ne,$Tf){list($Nf,$kh)=queries(null);return
query_redirect($Nf,$A,$ne,$Tf,false,!$Tf,$kh);}function
format_time($Jg){return
lang(1,max(0,microtime(true)-$Jg));}function
...
null;foreach($Ic
as$x=>$X)$Ic[$x]=(array)$X;$J='';foreach($Ic["error"]as$x=>$n){if($n)return$n;$C=$Ic["name"][$x];$sh=$Ic["tmp_name"][$x];$wb=file_get_contents($Lb&&preg_match('~\\.gz$~',$C)?"compress.zlib://$sh":$sh);if($Lb){$Jg=substr($wb,0,3);if(function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~",$Jg,$Zf))$wb=iconv("utf-16","utf-8",$wb);elseif($Jg=="\xEF\xBB\xBF")$wb=substr($wb,3);$J.=$wb."\n\n";}else$J.=$wb;}return$J;}function
...
shorten_utf8($P,$y=80,$Qg=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{FFFF}]",$y).")($)?)u",$P,$B))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$y).")($)?)",$P,$B);return
h($B[1]).$Qg.(isset($B[2])?"":"<i>...</i>");}function
...
enum_input($U,$Ja,$o,$Y,$nc=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$fe);$J=($nc!==null?"<label><input type='$U'$Ja value='$nc'".((is_array($Y)?in_array($nc,$Y):$Y===0)?" checked":"")."><i>".lang(7)."</i></label>":"");foreach($fe[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$db=(is_int($Y)?$Y==$s+1:(is_array($Y)?in_array($s+1,$Y):$Y===$X));$J.=" <label><input type='$U'$Ja value='".($s+1)."'".($db?' checked':'').'>'.h($b->editVal($X,$o)).'</label>';}return$J;}function
...
select_value($X,$_,$o,$jh){global$b,$ba;if(is_array($X)){$J="";foreach($X
as$Hd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($Hd):"")."<td>".select_value($W,$_,$o,$jh);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$o);if($_===null){if(is_mail($X))$_="mailto:$X";if($Lf=is_url($X))$_=(($Lf=="http"&&$ba)||preg_match('~WebKit~i',$_SERVER["HTTP_USER_AGENT"])?$X:"https://www.adminer.org/redirect/?url=".urlencode($X));}$J=$b->editVal($X,$o);if($J!==null){if($J==="")$J="&nbsp;";elseif(!is_utf8($J))$J="\0";elseif($jh!=""&&is_shortable($o))$J=shorten_utf8($J,max(0,+$jh));else$J=h($J);}return$b->selectVal($J,$_,$o,$X);}function
is_mail($kc){$Ha='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$Wb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$uf="$Ha+(\\.$Ha+)*@($Wb?\\.)+$Wb";return
is_string($kc)&&preg_match("(^$uf(,\\s*$uf)*\$)i",$kc);}function
...
slow_query($H){global$b,$T;$m=$b->database();$lh=$b->queryTimeout();if(support("kill")&&is_object($i=connect())&&($m==""||$i->select_db($m))){$Md=$i->result("SELECT CONNECTION_ID()");echo'<script type="text/javascript">
var timeout = setTimeout(function () {
	ajax(\'',js_escape(ME),'script=kill\', function () {
	}, \'token=',$T,'&kill=',$Md,'\');
}, ',1000*$lh,');
</script>
';}else$i=null;ob_flush();flush();$J=@get_key_vals($H,$i,$lh);if($i){echo"<script type='text/javascript'>clearTimeout(timeout);</script>\n";ob_flush();flush();}return
...
lzw_decompress($Ra){$Sb=256;$Sa=8;$kb=array();$eg=0;$fg=0;for($s=0;$s<strlen($Ra);$s++){$eg=($eg<<8)+ord($Ra[$s]);$fg+=8;if($fg>=$Sa){$fg-=$Sa;$kb[]=$eg>>$fg;$eg&=(1<<$fg)-1;$Sb++;if($Sb>>$Sa)$Sa++;}}$Rb=range("\0","\xFF");$J="";foreach($kb
as$s=>$jb){$jc=$Rb[$jb];if(!isset($jc))$jc=$fi.$fi[0];$J.=$jc;if($s)$Rb[]=$fi.$jc[0];$fi=$jc;}return$J;}function
on_help($pb,$Bg=0){return" onmouseover='helpMouseover(this, event, ".h($pb).", $Bg);' onmouseout='helpMouseout(this, event);'";}function
edit_form($a,$p,$K,$Lh){global$b,$w,$T,$n;$Vg=$b->tableName(table_status1($a,true));page_header(($Lh?lang(10):lang(11)),$n,array("select"=>array($a,$Vg)),$Vg);if($K===false)echo"<p class='error'>".lang(12)."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
';if(!$p)echo"<p class='error'>".lang(13)."\n";else{echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($p
as$C=>$o){echo"<tr><th>".$b->fieldName($o);$Mb=$_GET["set"][bracket_escape($C)];if($Mb===null){$Mb=$o["default"];if($o["type"]=="bit"&&preg_match("~^b'([01]*)'\$~",$Mb,$Zf))$Mb=$Zf[1];}$Y=($K!==null?($K[$C]!=""&&$w=="sql"&&preg_match("~enum|set~",$o["type"])?(is_array($K[$C])?array_sum($K[$C]):+$K[$C]):$K[$C]):(!$Lh&&$o["auto_increment"]?"":(isset($_GET["select"])?false:$Mb)));if(!$_POST["save"]&&is_string($Y))$Y=$b->editVal($Y,$o);$r=($_POST["save"]?(string)$_POST["function"][$C]:($Lh&&$o["on_update"]=="CURRENT_TIMESTAMP"?"now":($Y===false?null:($Y!==null?'':'NULL'))));if(preg_match("~time~",$o["type"])&&$Y=="CURRENT_TIMESTAMP"){$Y="";$r="now";}input($o,$Y,$r);echo"\n";}if(!support("table"))echo"<tr>"."<th><input name='field_keys[]' onkeyup='keyupChange.call(this);' onchange='fieldChange(this);' value=''>"."<td class='function'>".html_select("field_funs[]",$b->editFunctions(array("null"=>isset($_GET["select"]))))."<td><input name='field_vals[]'>"."\n";echo"</table>\n";}echo"<p>\n";if($p){echo"<input type='submit' value='".lang(14)."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Lh?lang(15)."' onclick='return !ajaxForm(this.form, \"".lang(16).'...", this)':lang(17))."' title='Ctrl+Shift+Enter'>\n";}echo($Lh?"<input type='submit' name='delete' value='".lang(18)."'".confirm().">\n":($_POST||!$p?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$T,'">
</form>
';}global$b,$h,$Yb,$gc,$qc,$n,$Vc,$ad,$ba,$vd,$w,$ca,$Qd,$Oe,$vf,$Ng,$ed,$T,$xh,$Dh,$Kh,$ia;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$ba=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);session_cache_limiter("");if(!defined("SID")){session_name("adminer_sid");$F=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$ba);if(version_compare(PHP_VERSION,'5.2.0')>=0)$F[]=true;call_user_func_array('session_set_cookie_params',$F);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Kc);if(get_magic_quotes_runtime())set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);$Qd=array('en'=>'English','ar'=>'Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©','bg'=>'Ð‘ÑŠÐ»Ð³Ð°Ñ€ÑÐºÐ¸','bn'=>'à¦¬à¦¾à¦‚à¦²à¦¾','ca'=>'CatalÃ ','cs'=>'ÄŒeÅ¡tina','da'=>'Dansk','de'=>'Deutsch','el'=>'Î•Î»Î»Î·Î½Î¹ÎºÎ¬','es'=>'EspaÃ±ol','et'=>'Eesti','fa'=>'ÙØ§Ø±Ø³ÛŒ','fr'=>'FranÃ§ais','gl'=>'Galego','hu'=>'Magyar','id'=>'Bahasa Indonesia','it'=>'Italiano','ja'=>'æ—¥æœ¬èªž','ko'=>'í•œêµ­ì–´','lt'=>'LietuviÅ³','nl'=>'Nederlands','no'=>'Norsk','pl'=>'Polski','pt'=>'PortuguÃªs','pt-br'=>'PortuguÃªs (Brazil)','ro'=>'Limba RomÃ¢nÄƒ','ru'=>'Ð ÑƒÑÑÐºÐ¸Ð¹ ÑÐ·Ñ‹Ðº','sk'=>'SlovenÄina','sl'=>'Slovenski','sr'=>'Ð¡Ñ€Ð¿ÑÐºÐ¸','ta'=>'à®¤â€Œà®®à®¿à®´à¯','th'=>'à¸ à¸²à¸©à¸²à¹„à¸—à¸¢','tr'=>'TÃ¼rkÃ§e','uk'=>'Ð£ÐºÑ€Ð°Ñ—Ð½ÑÑŒÐºÐ°','vi'=>'Tiáº¿ng Viá»‡t','zh'=>'ç®€ä½“ä¸­æ–‡','zh-tw'=>'ç¹é«”ä¸­æ–‡',);function
get_lang(){global$ca;return$ca;}function
lang($t,$Fe=null){if(is_string($t)){$yf=array_search($t,get_translations("en"));if($yf!==false)$t=$yf;}global$ca,$xh;$wh=($xh[$t]?$xh[$t]:$t);if(is_array($wh)){$yf=($Fe==1?0:($ca=='cs'||$ca=='sk'?($Fe&&$Fe<5?1:2):($ca=='fr'?(!$Fe?0:1):($ca=='pl'?($Fe%10>1&&$Fe%10<5&&$Fe/10%10!=1?1:2):($ca=='sl'?($Fe%100==1?0:($Fe%100==2?1:($Fe%100==3||$Fe%100==4?2:3))):($ca=='lt'?($Fe%10==1&&$Fe%100!=11?0:($Fe%10>1&&$Fe/10%10!=1?1:2)):($ca=='ru'||$ca=='sr'||$ca=='uk'?($Fe%10==1&&$Fe%100!=11?0:($Fe%10>1&&$Fe%10<5&&$Fe/10%10!=1?1:2)):1)))))));$wh=$wh[$yf];}$Ea=func_get_args();array_shift($Ea);$Qc=str_replace("%d","%s",$wh);if($Qc!=$wh)$Ea[0]=format_number($Fe);return
...
query($H,$Eh=false){$I=parent::query($H);$this->error="";if(!$I){list(,$this->errno,$this->error)=$this->errorInfo();return
...
rollback(){return
queries("ROLLBACK");}}$Yb["sqlite"]="SQLite 3";$Yb["sqlite2"]="SQLite 2";if(isset($_GET["sqlite"])||isset($_GET["sqlite2"])){$Af=array((isset($_GET["sqlite"])?"SQLite3":"SQLite"),"PDO_SQLite");define("DRIVER",(isset($_GET["sqlite"])?"sqlite":"sqlite2"));if(class_exists(isset($_GET["sqlite"])?"SQLite3":"SQLiteDatabase")){if(isset($_GET["sqlite"])){class
Min_SQLite{var$extension="SQLite3",$server_info,$affected_rows,$errno,$error,$_link;function
__construct($Jc){$this->_link=new
SQLite3($Jc);$Yh=$this->_link->version();$this->server_info=$Yh["versionString"];}function
...
query($H,$Eh=false){$te=($Eh?"unbufferedQuery":"query");$I=@$this->_link->$te($H,SQLITE_BOTH,$n);$this->error="";if(!$I){$this->error=$n;return
...
extends
Min_SQL{function
insertUpdate($Q,$L,$Df){$Vh=array();foreach($L
as$O)$Vh[]="(".implode(", ",$O).")";return
...
fields($Q){global$h;$J=array();$Df="";foreach(get_rows("PRAGMA table_info(".table($Q).")")as$K){$C=$K["name"];$U=strtolower($K["type"]);$Mb=$K["dflt_value"];$J[$C]=array("field"=>$C,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$Mb,$B)?str_replace("''","'",$B[1]):($Mb=="NULL"?null:$Mb)),"null"=>!$K["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$K["pk"],);if($K["pk"]){if($Df!="")$J[$Df]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$J[$C]["auto_increment"]=true;$Df=$C;}}$Hg=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($Q));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$Hg,$fe,PREG_SET_ORDER);foreach($fe
as$B){$C=str_replace('""','"',preg_replace('~^"|"$~','',$B[1]));if($J[$C])$J[$C]["collation"]=trim($B[3],"'");}return$J;}function
indexes($Q,$i=null){global$h;if(!is_object($i))$i=$h;$J=array();$Hg=$i->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($Q));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i',$Hg,$B)){$J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$B[1],$fe,PREG_SET_ORDER);foreach($fe
as$B){$J[""]["columns"][]=idf_unescape($B[2]).$B[4];$J[""]["descs"][]=(preg_match('~DESC~i',$B[5])?'1':null);}}if(!$J){foreach(fields($Q)as$C=>$o){if($o["primary"])$J[""]=array("type"=>"PRIMARY","columns"=>array($C),"lengths"=>array(),"descs"=>array(null));}}$Ig=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($Q),$i);foreach(get_rows("PRAGMA index_list(".table($Q).")",$i)as$K){$C=$K["name"];$u=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$u["lengths"]=array();$u["descs"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($C).")",$i)as$mg){$u["columns"][]=$mg["name"];$u["descs"][]=null;}if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($Q),'~').' \((.*)\)$~i',$Ig[$C],$Zf)){preg_match_all('/("[^"]*+")+( DESC)?/',$Zf[2],$fe);foreach($fe[2]as$x=>$X){if($X)$u["descs"][$x]='1';}}if(!$J[""]||$u["type"]!="UNIQUE"||$u["columns"]!=$J[""]["columns"]||$u["descs"]!=$J[""]["descs"]||!preg_match("~^sqlite_~",$C))$J[$C]=$u;}return$J;}function
...
alter_table($Q,$C,$p,$Nc,$rb,$oc,$d,$La,$pf){$Ph=($Q==""||$Nc);foreach($p
as$o){if($o[0]!=""||!$o[1]||$o[2]){$Ph=true;break;}}$c=array();$gf=array();foreach($p
as$o){if($o[1]){$c[]=($Ph?$o[1]:"ADD ".implode($o[1]));if($o[0]!="")$gf[$o[0]]=$o[1][0];}}if(!$Ph){foreach($c
...
as$o){if($o[6])$Ef=true;}$bc=array();foreach($v
as$x=>$X){if($X[2]=="DROP"){$bc[$X[1]]=true;unset($v[$x]);}}foreach(indexes($Q)as$Kd=>$u){$f=array();foreach($u["columns"]as$x=>$e){if(!$gf[$e])continue
2;$f[]=$gf[$e].($u["descs"][$x]?" DESC":"");}if(!$bc[$Kd]){if($u["type"]!="PRIMARY"||!$Ef)$v[]=array($u["type"],$Kd,$f);}}foreach($v
...
false;$Ah=array();foreach(triggers($Q)as$zh=>$mh){$yh=trigger($zh);$Ah[]="CREATE TRIGGER ".idf_escape($zh)." ".implode(" ",$mh)." ON ".table($C)."\n$yh[Statement]";}if(!queries("DROP TABLE ".table($Q)))return
...
drop_views($ai){return
apply_queries("DROP VIEW",$ai);}function
drop_tables($S){return
apply_queries("DROP TABLE",$S);}function
move_tables($S,$ai,$dh){return
false;}function
trigger($C){global$h;if($C=="")return
array("Statement"=>"BEGIN\n\t;\nEND");$t='(?:[^`"\\s]+|`[^`]*`|"[^"]*")+';$_h=trigger_options();preg_match("~^CREATE\\s+TRIGGER\\s*$t\\s*(".implode("|",$_h["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($t))?\\s+ON\\s*$t\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is",$h->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($C)),$B);$He=$B[3];return
array("Timing"=>strtoupper($B[1]),"Event"=>strtoupper($B[2]).($He?" OF":""),"Of"=>($He[0]=='`'||$He[0]=='"'?idf_unescape($He):$He),"Trigger"=>$C,"Statement"=>$B[4],);}function
triggers($Q){$J=array();$_h=trigger_options();foreach(get_rows("SELECT * FROM sqlite_master WHERE type = 'trigger' AND tbl_name = ".q($Q))as$K){preg_match('~^CREATE\\s+TRIGGER\\s*(?:[^`"\\s]+|`[^`]*`|"[^"]*")+\\s*('.implode("|",$_h["Timing"]).')\\s*(.*)\\s+ON\\b~iU',$K["sql"],$B);$J[$K["name"]]=array($B[1],$B[2]);}return$J;}function
...
query($H,$Eh=false){$I=@pg_query($this->_link,$H);$this->error="";if(!$I){$this->error=pg_last_error($this->_link);return
...
as$O){$Lh=array();$Z=array();foreach($O
as$x=>$X){$Lh[]="$x = $X";if(isset($Df[idf_unescape($x)]))$Z[]="$x = $X";}if(!(($Z&&queries("UPDATE ".table($Q)." SET ".implode(", ",$Lh)." WHERE ".implode(" AND ",$Z))&&$h->affected_rows)||queries("INSERT INTO ".table($Q)." (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).")")))return
false;}return
true;}}function
idf_escape($t){return'"'.str_replace('"','""',$t).'"';}function
table($t){return
...
indexes($Q,$i=null){global$h;if(!is_object($i))$i=$h;$J=array();$Wg=$i->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($Q));$f=get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $Wg AND attnum > 0",$i);foreach(get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption FROM pg_index i, pg_class ci WHERE i.indrelid = $Wg AND ci.oid = i.indexrelid",$i)as$K){$ag=$K["relname"];$J[$ag]["type"]=($K["indisprimary"]?"PRIMARY":($K["indisunique"]?"UNIQUE":"INDEX"));$J[$ag]["columns"]=array();foreach(explode(" ",$K["indkey"])as$rd)$J[$ag]["columns"][]=$f[$rd];$J[$ag]["descs"]=array();foreach(explode(" ",$K["indoption"])as$sd)$J[$ag]["descs"][]=($sd&1?'1':null);$J[$ag]["lengths"]=array();}return$J;}function
...
alter_table($Q,$C,$p,$Nc,$rb,$oc,$d,$La,$pf){$c=array();$Nf=array();foreach($p
as$o){$e=idf_escape($o[0]);$X=$o[1];if(!$X)$c[]="DROP $e";else{$Uh=$X[5];unset($X[5]);if(isset($X[6])&&$o[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($o[0]=="")$c[]=($Q!=""?"ADD ":"  ").implode($X);else{if($e!=$X[0])$Nf[]="ALTER TABLE ".table($Q)." RENAME $e TO $X[0]";$c[]="ALTER $e TYPE$X[1]";if(!$X[6]){$c[]="ALTER $e ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $e ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($o[0]!=""||$Uh!="")$Nf[]="COMMENT ON COLUMN ".table($Q).".$X[0] IS ".($Uh!=""?substr($Uh,9):"''");}}$c=array_merge($c,$Nc);if($Q=="")array_unshift($Nf,"CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($Nf,"ALTER TABLE ".table($Q)."\n".implode(",\n",$c));if($Q!=""&&$Q!=$C)$Nf[]="ALTER TABLE ".table($Q)." RENAME TO ".table($C);if($Q!=""||$rb!="")$Nf[]="COMMENT ON TABLE ".table($C)." IS ".q($rb);if($La!=""){}foreach($Nf
as$H){if(!queries($H))return
false;}return
true;}function
alter_indexes($Q,$c){$j=array();$Zb=array();$Nf=array();foreach($c
as$X){if($X[0]!="INDEX")$j[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"")."(".implode(", ",$X[2]).")");elseif($X[2]=="DROP")$Zb[]=idf_escape($X[1]);else$Nf[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($Q."_"))." ON ".table($Q)." (".implode(", ",$X[2]).")";}if($j)array_unshift($Nf,"ALTER TABLE ".table($Q).implode(",",$j));if($Zb)array_unshift($Nf,"DROP INDEX ".implode(", ",$Zb));foreach($Nf
as$H){if(!queries($H))return
false;}return
true;}function
truncate_tables($S){return
...
true;}function
drop_views($ai){return
drop_tables($ai);}function
drop_tables($S){foreach($S
as$Q){$Kg=table_status($Q);if(!queries("DROP ".strtoupper($Kg["Engine"])." ".table($Q)))return
false;}return
true;}function
move_tables($S,$ai,$dh){foreach(array_merge($S,$ai)as$Q){$Kg=table_status($Q);if(!queries("ALTER ".strtoupper($Kg["Engine"])." ".table($Q)." SET SCHEMA ".idf_escape($dh)))return
false;}return
true;}function
...
set_schema($pg){global$h,$Dh,$Ng;$J=$h->query("SET search_path TO ".idf_escape($pg));foreach(types()as$U){if(!isset($Dh[$U])){$Dh[$U]=0;$Ng[lang(23)][]=$U;}}return$J;}function
...
select_db($Gb){return
true;}function
query($H,$Eh=false){$I=oci_parse($this->_link,$H);$this->error="";if(!$I){$n=oci_error($this->_link);$this->errno=$n["code"];$this->error=$n["message"];return
...
found_rows($R,$Z){}function
alter_table($Q,$C,$p,$Nc,$rb,$oc,$d,$La,$pf){$c=$Zb=array();foreach($p
as$o){$X=$o[1];if($X&&$o[0]!=""&&idf_escape($o[0])!=$X[0])queries("ALTER TABLE ".table($Q)." RENAME COLUMN ".idf_escape($o[0])." TO $X[0]");if($X)$c[]=($Q!=""?($o[0]!=""?"MODIFY (":"ADD ("):"  ").implode($X).($Q!=""?")":"");else$Zb[]=idf_escape($o[0]);}if($Q=="")return
...
unconvert_field($o,$J){return$J;}function
support($Gc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Gc);}$w="oracle";$Dh=array();$Ng=array();foreach(array(lang(24)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(25)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(26)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(27)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$Dh+=$X;$Ng[$x]=array_keys($X);}$Kh=array();$Te=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Vc=array("length","lower","round","upper");$ad=array("avg","count","count distinct","max","min","sum");$gc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Yb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Af=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
query($H,$Eh=false){$I=sqlsrv_query($this->_link,$H);$this->error="";if(!$I){$this->_get_error();return
...
query($H,$Eh=false){$I=mssql_query($H,$this->_link);$this->error="";if(!$I){$this->error=mssql_get_last_message();return
...
as$O){$Lh=array();$Z=array();foreach($O
as$x=>$X){$Lh[]="$x = $X";if(isset($Df[idf_unescape($x)]))$Z[]="$x = $X";}if(!queries("MERGE ".table($Q)." USING (VALUES(".implode(", ",$O).")) AS source (c".implode(", c",range(1,count($O))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$Lh)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).");"))return
false;}return
true;}function
begin(){return
...
alter_table($Q,$C,$p,$Nc,$rb,$oc,$d,$La,$pf){$c=array();foreach($p
...
apply_queries("TRUNCATE TABLE",$S);}function
drop_views($ai){return
queries("DROP VIEW ".implode(", ",array_map('table',$ai)));}function
drop_tables($S){return
queries("DROP TABLE ".implode(", ",array_map('table',$S)));}function
move_tables($S,$ai,$dh){return
apply_queries("ALTER SCHEMA ".idf_escape($dh)." TRANSFER",array_merge($S,$ai));}function
trigger($C){if($C=="")return
...
true;}function
support($Gc){return
preg_match("~^(columns|sql|status|table)$~",$Gc);}$w="firebird";$Te=array("=");$Vc=array();$ad=array();$gc=array();}$Yb["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$Af=array("SimpleXML");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')){class
Min_DB{var$extension="SimpleXML",$server_info='2009-04-15',$error,$timeout,$next,$affected_rows,$_result;function
select_db($Gb){return($Gb=="domain");}function
query($H,$Eh=false){$F=array('SelectExpression'=>$H,'ConsistentRead'=>'true');if($this->next)$F['NextToken']=$this->next;$I=sdb_request_all('Select','Item',$F,$this->timeout);if($I===false)return$I;if(preg_match('~^\s*SELECT\s+COUNT\(~i',$H)){$Rg=0;foreach($I
as$Fd)$Rg+=$Fd->Attribute->Value;$I=array((object)array('Attribute'=>array((object)array('Name'=>'Count','Value'=>$Rg,))));}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
store_result(){return$this->_result;}function
...
update($Q,$O,$Of,$z=0,$wg="\n"){$Nb=array();$xd=array();$s=0;$md=$this->_extractIds($Q,$Of,$z);$kd=idf_unescape($O["`itemName()`"]);unset($O["`itemName()`"]);foreach($O
as$x=>$X){$x=idf_unescape($x);if($X=="NULL"||($kd!=""&&array($kd)!=$md))$Nb["Attribute.".count($Nb).".Name"]=$x;if($X!="NULL"){foreach((array)$X
as$Hd=>$W){$xd["Attribute.$s.Name"]=$x;$xd["Attribute.$s.Value"]=(is_array($X)?$W:idf_unescape($W));if(!$Hd)$xd["Attribute.$s.Replace"]="true";$s++;}}}$F=array('DomainName'=>$Q);return(!$xd||$this->_chunkRequest(($kd!=""?array($kd):$md),'BatchPutAttributes',$F,$xd))&&(!$Nb||$this->_chunkRequest($md,'BatchDeleteAttributes',$F,$Nb));}function
...
engines(){return
array();}function
alter_table($Q,$C,$p,$Nc,$rb,$oc,$d,$La,$pf){return($Q==""&&sdb_request('CreateDomain',array('DomainName'=>$C)));}function
drop_tables($S){foreach($S
...
false;}libxml_use_internal_errors(true);$gi=simplexml_load_string($Ic);if(!$gi){$n=libxml_get_last_error();$h->error=$n->message;return
false;}if($gi->Errors){$n=$gi->Errors->Error;$h->error="$n->Message ($n->Code)";return
false;}$h->error='';$ch=$va."Result";return($gi->$ch?$gi->$ch:true);}function
sdb_request_all($va,$ch,$F=array(),$lh=0){$J=array();$Jg=($lh?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$F['SelectExpression'],$B)?$B[1]:0);do{$gi=sdb_request($va,$F);if(!$gi)break;foreach($gi->$ch
as$jc)$J[]=$jc;if($z&&count($J)>=$z){$_GET["next"]=$gi->NextToken;break;}if($lh&&microtime(true)-$Jg>$lh)return
false;$F['NextToken']=$gi->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($gi->NextToken);return$J;}$w="simpledb";$Te=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$Vc=array();$ad=array("count");$gc=array(array("json"));}$Yb["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$Af=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
...
select($Q,$M,$Z,$Yc,$Ye=array(),$z=1,$E=0,$Ff=false){$M=($M==array("*")?array():array_fill_keys($M,true));$Eg=array();foreach($Ye
as$X){$X=preg_replace('~ DESC$~','',$X,1,$Ab);$Eg[$X]=($Ab?-1:1);}return
new
Min_Result($this->_conn->_db->selectCollection($Q)->find(array(),$M)->sort($Eg)->limit(+$z)->skip($E*$z));}function
...
h($h->error);}function
logged_user(){global$b;$k=$b->credentials();return$k[1];}function
get_databases($Mc){global$h;$J=array();$Kb=$h->_link->listDBs();foreach($Kb['databases']as$m)$J[]=$m['name'];return$J;}function
collations(){return
array();}function
db_collation($m,$nb){}function
count_tables($l){global$h;$J=array();foreach($l
...
alter_table($Q,$C,$p,$Nc,$rb,$oc,$d,$La,$pf){global$h;if($Q==""){$h->_db->createCollection($C);return
true;}}function
...
support($Gc){return
preg_match("~database|indexes~",$Gc);}$w="mongo";$Te=array("=");$Vc=array();$ad=array();$gc=array(array("json"));}$Yb["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$Af=array("json");define("DRIVER","elastic");if(function_exists('json_decode')){class
...
extends
Min_SQL{function
select($Q,$M,$Z,$Yc,$Ye=array(),$z=1,$E=0,$Ff=false){global$b;$Eb=array();$H="$Q/_search";if($M!=array("*"))$Eb["fields"]=$M;if($Ye){$Eg=array();foreach($Ye
as$lb){$lb=preg_replace('~ DESC$~','',$lb,1,$Ab);$Eg[]=($Ab?array($lb=>"desc"):$lb);}$Eb["sort"]=$Eg;}if($z){$Eb["size"]=+$z;if($E)$Eb["from"]=($E*$z);}foreach($Z
as$X){list($lb,$Re,$X)=explode(" ",$X,3);if($lb=="_id")$Eb["query"]["ids"]["values"][]=$X;elseif($lb.$X!=""){$gh=array("term"=>array(($lb!=""?$lb:"_all")=>$X));if($Re=="=")$Eb["query"]["filtered"]["filter"]["and"][]=$gh;else$Eb["query"]["filtered"]["query"]["bool"]["must"][]=$gh;}}if($Eb["query"]&&!$Eb["query"]["filtered"]["query"]&&!$Eb["query"]["ids"])$Eb["query"]["filtered"]["query"]=array("match_all"=>array());$Jg=microtime(true);$rg=$this->_conn->query($H,$Eb);if($Ff)echo$b->selectQuery("$H: ".print_r($Eb,true),format_time($Jg));if(!$rg)return
...
as$Q)$J=$J&&$h->query(urlencode($Q),array(),'DELETE');return$J;}$w="elastic";$Te=array("=","query");$Vc=array();$ad=array();$gc=array(array("json"));}$Yb=array("server"=>"MySQL")+$Yb;if(!defined("DRIVER")){$Af=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
MySQLi{var$extension="MySQLi";function
__construct(){parent::init();}function
connect($N=NULL,$V=NULL,$G=NULL,$Jb=NULL,$xf=NULL,$Dg=NULL){mysqli_report(MYSQLI_REPORT_OFF);list($id,$xf)=explode(":",$N,2);$J=@$this->real_connect(($N!=""?$id:ini_get("mysqli.default_host")),($N.$V!=""?$V:ini_get("mysqli.default_user")),($N.$V.$G!=""?$G:ini_get("mysqli.default_pw")),null,(is_numeric($xf)?$xf:ini_get("mysqli.default_port")),(!is_numeric($xf)?$xf:null));return$J;}function
...
query($H,$Eh=false){$I=@($Eh?mysql_unbuffered_query($H,$this->_link):mysql_query($H,$this->_link));$this->error="";if(!$I){$this->errno=mysql_errno($this->_link);$this->error=mysql_error($this->_link);return
...
select_db($Gb){return$this->query("USE ".idf_escape($Gb));}function
query($H,$Eh=false){$this->setAttribute(1000,!$Eh);return
parent::query($H,$Eh);}}}class
Min_Driver
extends
Min_SQL{function
insert($Q,$O){return($O?parent::insert($Q,$O):queries("INSERT INTO ".table($Q)." ()\nVALUES ()"));}function
insertUpdate($Q,$L,$Df){$f=array_keys(reset($L));$Bf="INSERT INTO ".table($Q)." (".implode(", ",$f).") VALUES\n";$Vh=array();foreach($f
as$x)$Vh[$x]="$x = VALUES($x)";$Qg="\nON DUPLICATE KEY UPDATE ".implode(", ",$Vh);$Vh=array();$y=0;foreach($L
as$O){$Y="(".implode(", ",$O).")";if($Vh&&(strlen($Bf)+$y+strlen($Y)+strlen($Qg)>1e6)){if(!queries($Bf.implode(",\n",$Vh).$Qg))return
false;$Vh=array();$y=0;}$Vh[]=$Y;$y+=strlen($Y)+2;}return
queries($Bf.implode(",\n",$Vh).$Qg);}}function
idf_escape($t){return"`".str_replace("`","``",$t)."`";}function
table($t){return
idf_escape($t);}function
...
as$B){preg_match_all("~$uf~",$B[2],$Fg);preg_match_all("~$uf~",$B[5],$dh);$J[idf_unescape($B[1])]=array("db"=>idf_unescape($B[4]!=""?$B[3]:$B[4]),"table"=>idf_unescape($B[4]!=""?$B[4]:$B[3]),"source"=>array_map('idf_unescape',$Fg[0]),"target"=>array_map('idf_unescape',$dh[0]),"on_delete"=>($B[6]?$B[6]:"RESTRICT"),"on_update"=>($B[7]?$B[7]:"RESTRICT"),);}}return$J;}function
view($C){global$h;return
...
alter_table($Q,$C,$p,$Nc,$rb,$oc,$d,$La,$pf){$c=array();foreach($p
as$o)$c[]=($o[1]?($Q!=""?($o[0]!=""?"CHANGE ".idf_escape($o[0]):"ADD"):" ")." ".implode($o[1]).($Q!=""?$o[2]:""):"DROP ".idf_escape($o[0]));$c=array_merge($c,$Nc);$Kg=($rb!==null?" COMMENT=".q($rb):"").($oc?" ENGINE=".q($oc):"").($d?" COLLATE ".q($d):"").($La!=""?" AUTO_INCREMENT=$La":"");if($Q=="")return
queries("CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)$Kg$pf");if($Q!=$C)$c[]="RENAME TO ".table($C);if($Kg)$c[]=ltrim($Kg);return($c||$pf?queries("ALTER TABLE ".table($Q)."\n".implode(",\n",$c).$pf):true);}function
alter_indexes($Q,$c){foreach($c
...
drop_tables($S){return
queries("DROP TABLE ".implode(", ",array_map('table',$S)));}function
move_tables($S,$ai,$dh){$bg=array();foreach(array_merge($S,$ai)as$Q)$bg[]=table($Q)." TO ".idf_escape($dh).".".table($Q);return
queries("RENAME TABLE ".implode(", ",$bg));}function
copy_tables($S,$ai,$dh){queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach($S
as$Q){$C=($dh==DB?table("copy_$Q"):idf_escape($dh).".".table($Q));if(!queries("\nDROP TABLE IF EXISTS $C")||!queries("CREATE TABLE $C LIKE ".table($Q))||!queries("INSERT INTO $C SELECT * FROM ".table($Q)))return
false;}foreach($ai
as$Q){$C=($dh==DB?table("copy_$Q"):idf_escape($dh).".".table($Q));$Zh=view($Q);if(!queries("DROP VIEW IF EXISTS $C")||!queries("CREATE VIEW $C AS $Zh[select]"))return
false;}return
true;}function
trigger($C){if($C=="")return
...
routine($C,$U){global$h,$qc,$vd,$Dh;$Ca=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Ch="((".implode("|",array_merge(array_keys($Dh),$Ca)).")\\b(?:\\s*\\(((?:[^'\")]|$qc)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$uf="\\s*(".($U=="FUNCTION"?"":$vd).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Ch";$j=$h->result("SHOW CREATE $U ".idf_escape($C),2);preg_match("~\\(((?:$uf\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Ch\\s+":"")."(.*)~is",$j,$B);$p=array();preg_match_all("~$uf\\s*,?~is",$B[1],$fe,PREG_SET_ORDER);foreach($fe
as$kf){$C=str_replace("``","`",$kf[2]).$kf[3];$p[]=array("field"=>$C,"type"=>strtolower($kf[5]),"length"=>preg_replace_callback("~$qc~s",'normalize_enum',$kf[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$kf[8] $kf[7]"))),"null"=>1,"full_type"=>$kf[4],"inout"=>strtoupper($kf[1]),"collation"=>strtolower($kf[9]),);}if($U!="FUNCTION")return
...
use_sql($Gb){return"USE ".idf_escape($Gb);}function
trigger_sql($Q,$Og){$J="";foreach(get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($Q,"%_\\")),null,"-- ")as$K)$J.="\n".($Og=='CREATE+ALTER'?"DROP TRIGGER IF EXISTS ".idf_escape($K["Trigger"]).";;\n":"")."CREATE TRIGGER ".idf_escape($K["Trigger"])." $K[Timing] $K[Event] ON ".table($K["Table"])." FOR EACH ROW\n$K[Statement];;\n";return$J;}function
show_variables(){return
...
selectLinks($Ug,$O=""){echo'<p class="links">';$ae=array("select"=>lang(38));if(support("table")||support("indexes"))$ae["table"]=lang(39);if(support("table")){if(is_view($Ug))$ae["view"]=lang(40);else$ae["create"]=lang(41);}if($O!==null)$ae["edit"]=lang(42);foreach($ae
as$x=>$X)echo" <a href='".h(ME)."$x=".urlencode($Ug["Name"]).($x=="edit"?$O:"")."'".bold(isset($_GET[$x])).">$X</a>";echo"\n";}function
foreignKeys($Q){return
foreign_keys($Q);}function
backwardKeys($Q,$Tg){return
array();}function
backwardKeysPrint($Oa,$K){}function
selectQuery($H,$kh){global$w;return"<p><code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <span class='time'>($kh)</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>":"")."</p>";}function
...
selectLengthPrint($jh){if($jh!==null){echo"<fieldset><legend>".lang(52)."</legend><div>","<input type='number' name='text_length' class='size' value='".h($jh)."'>","</div></fieldset>\n";}}function
...
messageQuery($H,$kh){global$w;restart_session();$fd=&get_session("queries");$kd="sql-".count($fd[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$fd[$_GET["db"]][]=array($H,time(),$kh);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$kd' onclick=\"return !toggle('$kd');\">".lang(55)."</a>"."<div id='$kd' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre>'.($kh?" <span class='time'>($kh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($fd[$_GET["db"]])-1)).'">'.lang(10).'</a>':'').'</div>';}function
...
dumpTable($Q,$Og,$Ed=0){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($Og)dump_csv(array_keys(fields($Q)));}else{if($Ed==2){$p=array();foreach(fields($Q)as$C=>$o)$p[]=idf_escape($C)." $o[full_type]";$j="CREATE TABLE ".table($Q)." (".implode(", ",$p).")";}else$j=create_sql($Q,$_POST["auto_increment"]);set_utf8mb4($j);if($Og&&$j){if($Og=="DROP+CREATE"||$Ed==1)echo"DROP ".($Ed==2?"VIEW":"TABLE")." IF EXISTS ".table($Q).";\n";if($Ed==1)$j=remove_definer($j);echo"$j;\n\n";}}}function
dumpData($Q,$Og,$H){global$h,$w;$he=($w=="sqlite"?0:1048576);if($Og){if($_POST["format"]=="sql"){if($Og=="TRUNCATE+INSERT")echo
truncate_sql($Q).";\n";$p=fields($Q);}$I=$h->query($H,1);if($I){$xd="";$Xa="";$Ld=array();$Qg="";$Hc=($Q!=''?'fetch_assoc':'fetch_row');while($K=$I->$Hc()){if(!$Ld){$Vh=array();foreach($K
as$X){$o=$I->fetch_field();$Ld[]=$o->name;$x=idf_escape($o->name);$Vh[]="$x = VALUES($x)";}$Qg=($Og=="INSERT+UPDATE"?"\nON DUPLICATE KEY UPDATE ".implode(", ",$Vh):"").";\n";}if($_POST["format"]!="sql"){if($Og=="table"){dump_csv($Ld);$Og="INSERT";}dump_csv($K);}else{if(!$xd)$xd="INSERT INTO ".table($Q)." (".implode(", ",array_map('idf_escape',$Ld)).") VALUES";foreach($K
as$x=>$X){$o=$p[$x];$K[$x]=($X!==null?unconvert_field($o,preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&$X!=''?$X:q($X)):"NULL");}$ng=($he?"\n":" ")."(".implode(",\t",$K).")";if(!$Xa)$Xa=$xd.$ng;elseif(strlen($Xa)+4+strlen($ng)+strlen($Qg)<$he)$Xa.=",$ng";else{echo$Xa.$Qg;$Xa=$xd.$ng;}}}if($Xa)echo$Xa.$Qg;}elseif($_POST["format"]=="sql")echo"-- ".str_replace("\n"," ",$h->error)."\n";}}function
...
true;}function
navigation($ve){global$ia,$w,$Yb,$h;echo'<h1>
...
';if($ve=="auth"){$Lc=true;foreach((array)$_SESSION["pwds"]as$Xh=>$_g){foreach($_g
as$N=>$Sh){foreach($Sh
as$V=>$G){if($G!==null){if($Lc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Lc=false;}$Kb=$_SESSION["db"][$Xh][$N][$V];foreach(($Kb?array_keys($Kb):array(""))as$m)echo"<a href='".h(auth_url($Xh,$N,$V,$m))."'>($Yb[$Xh]) ".h($V.($N!=""?"@$N":"").($m!=""?" - $m":""))."</a><br>\n";}}}}}else{if($_GET["ns"]!==""&&!$ve&&DB!=""){$h->select_db(DB);$S=table_status('',true);}if(support("sql")){echo'<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=jush.js&amp;version=4.2.3.1",'"></script>
<script type="text/javascript">
...
';}$this->databasesPrint($ve);if(DB==""||!$ve){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".lang(55)."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".lang(64)."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".lang(65)."</a>\n";}if($_GET["ns"]!==""&&!$ve&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".lang(66)."</a>\n";if(!$S)echo"<p class='message'>".lang(9)."\n";else$this->tablesPrint($S);}}}function
...
as$Q=>$Kg){echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q,"select").">".lang(69)."</a> ";$C=$this->tableName($Kg);echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($Kg)?"view":""),"structure")." title='".lang(39)."'>$C</a>":"<span>$C</span>")."<br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
Adminer);if($b->operators===null)$b->operators=$Te;function
page_header($nh,$n="",$Wa=array(),$oh=""){global$ca,$ia,$b,$Yb,$w;page_headers();if(is_ajax()&&$n){page_messages($n);exit;}$ph=$nh.($oh!=""?": $oh":"");$qh=strip_tags($ph.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE html>
...
<title>',$qh,'</title>
<link rel="stylesheet" type="text/css" href="',h(preg_replace("~\\?.*~","",ME))."?file=default.css&amp;version=4.2.3.1",'">
<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=functions.js&amp;version=4.2.3.1",'"></script>
';if($b->head()){echo'<link rel="shortcut icon" type="image/x-icon" href="',h(preg_replace("~\\?.*~","",ME))."?file=favicon.ico&amp;version=4.2.3.1",'">
<link rel="apple-touch-icon" href="',h(preg_replace("~\\?.*~","",ME))."?file=favicon.ico&amp;version=4.2.3.1",'">
...
as$x=>$X){$Pb=(is_array($X)?$X[1]:h($X));if($Pb!="")echo"<a href='".h(ME."$x=").urlencode(is_array($X)?$X[0]:$X)."'>$Pb</a> &raquo; ";}}echo"$nh\n";}}echo"<h2>$ph</h2>\n","<div id='ajaxstatus' class='jsonly hidden'></div>\n";restart_session();page_messages($n);$l=&get_session("dbs");if(DB!=""&&$l&&!in_array(DB,$l,true))$l=null;stop_session();define("PAGE_HEADER",1);}function
page_headers(){global$b;header("Content-Type: text/html; charset=utf-8");header("Cache-Control: no-cache");if($b->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}}function
page_messages($n){$Mh=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$re=$_SESSION["messages"][$Mh];if($re){echo"<div class='message'>".implode("</div>\n<div class='message'>",$re)."</div>\n";unset($_SESSION["messages"][$Mh]);}if($n)echo"<div class='error'>$n</div>\n";}function
...
long2str($W,$ci){$ng='';foreach($W
as$X)$ng.=pack('V',$X);if($ci)return
substr($ng,0,end($W));return$ng;}function
str2long($ng,$ci){$W=array_values(unpack('V*',str_pad($ng,4*ceil(strlen($ng)/4),"\0")));if($ci)$W[]=strlen($ng);return$W;}function
xxtea_mx($ii,$hi,$Rg,$Hd){return
int32((($ii>>5&0x7FFFFFF)^$hi<<2)+(($hi>>3&0x1FFFFFFF)^$ii<<4))^int32(($Rg^$hi)+($Hd^$ii));}function
encrypt_string($Mg,$x){if($Mg=="")return"";$x=array_values(unpack("V*",pack("H*",md5($x))));$W=str2long($Mg,true);$ye=count($W)-1;$ii=$W[$ye];$hi=$W[0];$Mf=floor(6+52/($ye+1));$Rg=0;while($Mf-->0){$Rg=int32($Rg+0x9E3779B9);$fc=$Rg>>2&3;for($jf=0;$jf<$ye;$jf++){$hi=$W[$jf+1];$xe=xxtea_mx($ii,$hi,$Rg,$x[$jf&3^$fc]);$ii=int32($W[$jf]+$xe);$W[$jf]=$ii;}$hi=$W[0];$xe=xxtea_mx($ii,$hi,$Rg,$x[$jf&3^$fc]);$ii=int32($W[$ye]+$xe);$W[$ye]=$ii;}return
long2str($W,false);}function
decrypt_string($Mg,$x){if($Mg=="")return"";if(!$x)return
false;$x=array_values(unpack("V*",pack("H*",md5($x))));$W=str2long($Mg,false);$ye=count($W)-1;$ii=$W[$ye];$hi=$W[0];$Mf=floor(6+52/($ye+1));$Rg=int32($Mf*0x9E3779B9);while($Rg){$fc=$Rg>>2&3;for($jf=$ye;$jf>0;$jf--){$ii=$W[$jf-1];$xe=xxtea_mx($ii,$hi,$Rg,$x[$jf&3^$fc]);$hi=int32($W[$jf]-$xe);$W[$jf]=$hi;}$ii=$W[$ye];$xe=xxtea_mx($ii,$hi,$Rg,$x[$jf&3^$fc]);$hi=int32($W[0]-$xe);$W[0]=$hi;$Rg=int32($Rg-0x9E3779B9);}return
...
add_invalid_login(){global$b;$Jc=get_temp_dir()."/adminer.invalid";$Tc=@fopen($Jc,"r+");if(!$Tc){$Tc=@fopen($Jc,"w");if(!$Tc)return;}flock($Tc,LOCK_EX);$_d=unserialize(stream_get_contents($Tc));$kh=time();if($_d){foreach($_d
as$Ad=>$X){if($X[0]<$kh)unset($_d[$Ad]);}}$zd=&$_d[$b->bruteForceKey()];if(!$zd)$zd=array($kh+30*60,0);$zd[1]++;$yg=serialize($_d);rewind($Tc);fwrite($Tc,$yg);ftruncate($Tc,strlen($yg));flock($Tc,LOCK_UN);fclose($Tc);}$Ka=$_POST["auth"];if($Ka){$_d=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$zd=$_d[$b->bruteForceKey()];$De=($zd[1]>30?$zd[0]-time():0);if($De>0)auth_error(lang(73,ceil($De/60)));session_regenerate_id();$Xh=$Ka["driver"];$N=$Ka["server"];$V=$Ka["username"];$G=(string)$Ka["password"];$m=$Ka["db"];set_password($Xh,$N,$V,$G);$_SESSION["db"][$Xh][$N][$V][$m]=true;if($Ka["permanent"]){$x=base64_encode($Xh)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($m);$Gf=$b->permanentLogin(true);$vf[$x]="$x:".base64_encode($Gf?encrypt_string($G,$Gf):"");cookie("adminer_permanent",implode(" ",$vf));}if(count($_POST)==1||DRIVER!=$Xh||SERVER!=$N||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($Xh,$N,$V,$m));}elseif($_POST["logout"]){if($ed&&!verify_token()){page_header(lang(72),lang(74));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(75));}}elseif($vf&&!$_SESSION["pwds"]){session_regenerate_id();$Gf=$b->permanentLogin();foreach($vf
as$x=>$X){list(,$hb)=explode(":",$X);list($Xh,$N,$V,$m)=array_map('base64_decode',explode("-",$x));set_password($Xh,$N,$V,decrypt_string(base64_decode($hb),$Gf));$_SESSION["db"][$Xh][$N][$V][$m]=true;}}function
unset_permanent(){global$vf;foreach($vf
as$x=>$X){list($Xh,$N,$V,$m)=array_map('base64_decode',explode("-",$x));if($Xh==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$m==DB)unset($vf[$x]);}cookie("adminer_permanent",implode(" ",$vf));}function
auth_error($n){global$b,$ed;$n=h($n);$Ag=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Ag]||$_GET[$Ag])&&!$ed)$n=lang(76);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$n.='<br>'.lang(77,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Ag]&&$_GET[$Ag]&&ini_bool("session.use_only_cookies"))$n=lang(78);$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(79),lang(80,implode(", ",$Af)),false);page_footer("auth");exit;}$h=connect();}$Xb=new
...
select($I,$i=null,$bf=array(),$z=0){global$w;$ae=array();$v=array();$f=array();$Ta=array();$Dh=array();$J=array();odd('');for($s=0;(!$z||$s<$z)&&($K=$I->fetch_row());$s++){if(!$s){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($Gd=0;$Gd<count($K);$Gd++){$o=$I->fetch_field();$C=$o->name;$af=$o->orgtable;$Ze=$o->orgname;$J[$o->table]=$af;if($bf&&$w=="sql")$ae[$Gd]=($C=="table"?"table=":($C=="possible_keys"?"indexes=":null));elseif($af!=""){if(!isset($v[$af])){$v[$af]=array();foreach(indexes($af,$i)as$u){if($u["type"]=="PRIMARY"){$v[$af]=array_flip($u["columns"]);break;}}$f[$af]=$v[$af];}if(isset($f[$af][$Ze])){unset($f[$af][$Ze]);$v[$af][$Ze]=$Gd;$ae[$Gd]=$af;}}if($o->charsetnr==63)$Ta[$Gd]=true;$Dh[$Gd]=$o->type;echo"<th".($af!=""||$o->name!=$Ze?" title='".h(($af!=""?"$af.":"").$Ze)."'":"").">".h($C).($bf?doc_link(array('sql'=>"explain-output.html#explain_".strtolower($C))):"");}echo"</thead>\n";}echo"<tr".odd().">";foreach($K
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ta[$x]&&!is_utf8($X))$X="<i>".lang(43,strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Dh[$x]==254)$X="<code>$X</code>";}if(isset($ae[$x])&&!$f[$ae[$x]]){if($bf&&$w=="sql"){$Q=$K[array_search("table=",$ae)];$_=$ae[$x].urlencode($bf[$Q]!=""?$bf[$Q]:$Q);}else{$_="edit=".urlencode($ae[$x]);foreach($v[$ae[$x]]as$lb=>$Gd)$_.="&where".urlencode("[".bracket_escape($lb)."]")."=".urlencode($K[$Gd]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($s?"</table>":"<p class='message'>".lang(12))."\n";return$J;}function
referencable_primary($vg){$J=array();foreach(table_status('',true)as$Vg=>$Q){if($Vg!=$vg&&fk_support($Q)){foreach(fields($Vg)as$o){if($o["primary"]){if($J[$Vg]){unset($J[$Vg]);break;}$J[$Vg]=$o;}}}}return$J;}function
...
edit_type($x,$o,$nb,$Pc=array()){global$Ng,$Dh,$Kh,$Oe;$U=$o["type"];echo'<td><select name="',h($x),'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($Dh[$U])&&!isset($Pc[$U]))array_unshift($Ng,$U);if($Pc)$Ng[lang(86)]=$Pc;echo
optionlist($Ng,$U),'</select>
<td><input name="',h($x),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='".h($x)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.lang(87).')'.optionlist($nb,$o["collation"]).'</select>',($Kh?"<select name='".h($x)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($Kh,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($x)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(88).")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($Pc?"<select name='".h($x)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".lang(89).")".optionlist(explode("|",$Oe),$o["on_delete"])."</select> ":" ");}function
process_length($y){global$qc;return(preg_match("~^\\s*\\(?\\s*$qc(?:\\s*,\\s*$qc)*+\\s*\\)?\\s*\$~",$y)&&preg_match_all("~$qc~",$y,$fe)?"(".implode(",",$fe[0]).")":preg_replace('~^[0-9].*~','(\0)',preg_replace('~[^-0-9,+()[\]]~','',$y)));}function
process_type($o,$mb="COLLATE"){global$Kh;return" $o[type]".process_length($o["length"]).(preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&in_array($o["unsigned"],$Kh)?" $o[unsigned]":"").(preg_match('~char|text|enum|set~',$o["type"])&&$o["collation"]?" $mb ".q($o["collation"]):"");}function
process_field($o,$Bh){global$w;$Mb=$o["default"];return
array(idf_escape(trim($o["field"])),process_type($Bh),($o["null"]?" NULL":" NOT NULL"),(isset($Mb)?" DEFAULT ".((preg_match('~time~',$o["type"])&&preg_match('~^CURRENT_TIMESTAMP$~i',$Mb))||($w=="sqlite"&&preg_match('~^CURRENT_(TIME|TIMESTAMP|DATE)$~i',$Mb))||($o["type"]=="bit"&&preg_match("~^([0-9]+|b'[0-1]+')\$~",$Mb))||($w=="pgsql"&&preg_match("~^[a-z]+\\(('[^']*')+\\)\$~",$Mb))?$Mb:q($Mb)):""),(preg_match('~timestamp|datetime~',$o["type"])&&$o["on_update"]?" ON UPDATE $o[on_update]":""),(support("comment")&&$o["comment"]!=""?" COMMENT ".q($o["comment"]):""),($o["auto_increment"]?auto_increment():null),);}function
...
<td><input type="radio" name="auto_increment_col" value=""><acronym title="',lang(56),'">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>',lang(95),(support("comment")?"<td".($sb?"":" class='hidden'").">".lang(96):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.3.1' alt='+' title='".lang(97)."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$s=>$o){$s++;$cf=$o[($_POST?"orig":"field")];$Tb=(isset($_POST["add"][$s-1])||(isset($o["field"])&&!$_POST["drop_col"][$s]))&&(support("drop_col")||$cf=="");echo'<tr',($Tb?"":" style='display: none;'"),'>
',($U=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$vd),$o["inout"]):""),'<th>';if($Tb){echo'<input name="fields[',$s,'][field]" value="',h($o["field"]),'" onchange="editingNameChange(this);',($o["field"]!=""||count($p)>1?'':' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'),'" maxlength="64" autocapitalize="off">';}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($cf),'">
...
',(support("comment")?"<td".($sb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".($h->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.3.1' alt='+' title='".lang(97)."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.2.3.1' alt='^' title='".lang(98)."'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.2.3.1' alt='v' title='".lang(99)."'>&nbsp;":""),($cf==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.2.3.1' alt='x' title='".lang(100)."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
drop_create($Zb,$j,$ac,$hh,$cc,$A,$qe,$oe,$pe,$Ke,$Ae){if($_POST["drop"])query_redirect($Zb,$A,$qe);elseif($Ke=="")query_redirect($j,$A,$pe);elseif($Ke!=$Ae){$Cb=queries($j);queries_redirect($A,$oe,$Cb&&queries($Zb));if($Cb)queries($ac);}else
queries_redirect($A,$oe,queries($hh)&&queries($cc)&&queries($Zb)&&queries($j));}function
create_trigger($Ne,$K){global$w;$mh=" $K[Timing] $K[Event]".($K["Event"]=="UPDATE OF"?" ".idf_escape($K["Of"]):"");return"CREATE TRIGGER ".idf_escape($K["Trigger"]).($w=="mssql"?$Ne.$mh:$mh.$Ne).rtrim(" $K[Type]\n$K[Statement]",";").";";}function
...
tar_file($Jc,$rh){$J=pack("a100a8a8a8a12a12",$Jc,644,0,0,decoct($rh->size),decoct(time()));$fb=8*32;for($s=0;$s<strlen($J);$s++)$fb+=ord($J[$s]);$J.=sprintf("%06o",$fb)."\0 ";echo$J,str_repeat("\0",512-strlen($J));$rh->send();echo
str_repeat("\0",511-($rh->size+511)%512);}function
ini_bytes($ud){$X=ini_get($ud);switch(strtolower(substr($X,-1))){case'g':$X*=1024;case'm':$X*=1024;case'k':$X*=1024;}return$X;}function
doc_link($tf){global$w,$h;$Oh=array('sql'=>"http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/",'sqlite'=>"http://www.sqlite.org/",'pgsql'=>"http://www.postgresql.org/docs/".substr($h->server_info,0,3)."/static/",'mssql'=>"http://msdn.microsoft.com/library/",'oracle'=>"http://download.oracle.com/docs/cd/B19306_01/server.102/b14200/",);return($tf[$w]?"<a href='$Oh[$w]$tf[$w]' target='_blank' rel='noreferrer'><sup>?</sup></a>":"");}function
...
as$C=>$q){echo"<tr title='".h($C)."'>","<th><i>".implode("</i>, <i>",array_map('h',$q["source"]))."</i>","<td><a href='".h($q["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($q["db"]),ME):($q["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($q["ns"]),ME):ME))."table=".urlencode($q["table"])."'>".($q["db"]!=""?"<b>".h($q["db"])."</b>.":"").($q["ns"]!=""?"<b>".h($q["ns"])."</b>.":"").h($q["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$q["target"]))."</i>)","<td>".nbsp($q["on_delete"])."\n","<td>".nbsp($q["on_update"])."\n",'<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($C)).'">'.lang(125).'</a>';}echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'foreign='.urlencode($a).'">'.lang(126)."</a>\n";}}if(support(is_view($R)?"view_trigger":"trigger")){echo"<h3 id='triggers'>".lang(127)."</h3>\n";$Ah=triggers($a);if($Ah){echo"<table cellspacing='0'>\n";foreach($Ah
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".lang(125)."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.lang(128)."</a>\n";}}elseif(isset($_GET["schema"])){page_header(lang(62),"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Xg=array();$Yg=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$fe,PREG_SET_ORDER);foreach($fe
as$s=>$B){$Xg[$B[1]]=array($B[2],$B[3]);$Yg[]="\n\t'".js_escape($B[1])."': [ $B[2], $B[3] ]";}$th=0;$Qa=-1;$pg=array();$Xf=array();$Vd=array();foreach(table_status('',true)as$Q=>$R){if(is_view($R))continue;$yf=0;$pg[$Q]["fields"]=array();foreach(fields($Q)as$C=>$o){$yf+=1.25;$o["pos"]=$yf;$pg[$Q]["fields"][$C]=$o;}$pg[$Q]["pos"]=($Xg[$Q]?$Xg[$Q]:array($th,0));foreach($b->foreignKeys($Q)as$X){if(!$X["db"]){$Td=$Qa;if($Xg[$Q][1]||$Xg[$X["table"]][1])$Td=min(floatval($Xg[$Q][1]),floatval($Xg[$X["table"]][1]))-1;else$Qa-=.1;while($Vd[(string)$Td])$Td-=.0001;$pg[$Q]["references"][$X["table"]][(string)$Td]=array($X["source"],$X["target"]);$Xf[$X["table"]][$Q][(string)$Td]=$X["target"];$Vd[(string)$Td]=true;}}$th=max($th,$pg[$Q]["pos"][0]+2.5+$yf);}echo'<div id="schema" style="height: ',$th,'em;" onselectstart="return false;">
<script type="text/javascript">
var tablePos = {',implode(",",$Yg)."\n",'};
var em = document.getElementById(\'schema\').offsetHeight / ',$th,';
...
';foreach($pg
as$C=>$Q){echo"<div class='table' style='top: ".$Q["pos"][0]."em; left: ".$Q["pos"][1]."em;' onmousedown='schemaMousedown(this, event);'>",'<a href="'.h(ME).'table='.urlencode($C).'"><b>'.h($C)."</b></a>";foreach($Q["fields"]as$o){$X='<span'.type_class($o["type"]).' title="'.h($o["full_type"].($o["null"]?" NULL":'')).'">'.h($o["field"]).'</span>';echo"<br>".($o["primary"]?"<i>$X</i>":$X);}foreach((array)$Q["references"]as$eh=>$Yf){foreach($Yf
as$Td=>$Uf){$Ud=$Td-$Xg[$C][1];$s=0;foreach($Uf[0]as$Fg)echo"\n<div class='references' title='".h($eh)."' id='refs$Td-".($s++)."' style='left: $Ud"."em; top: ".$Q["fields"][$Fg]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$Ud)."em;'></div></div>";}}foreach((array)$Xf[$C]as$eh=>$Yf){foreach($Yf
as$Td=>$f){$Ud=$Td-$Xg[$C][1];$s=0;foreach($f
as$dh)echo"\n<div class='references' title='".h($eh)."' id='refd$Td-".($s++)."' style='left: $Ud"."em; top: ".$Q["fields"][$dh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.2.3.1'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Ud)."em;'></div></div>";}}echo"\n</div>\n";}foreach($pg
as$C=>$Q){foreach((array)$Q["references"]as$eh=>$Yf){foreach($Yf
as$Td=>$Uf){$ue=$th;$je=-10;foreach($Uf[0]as$x=>$Fg){$zf=$Q["pos"][0]+$Q["fields"][$Fg]["pos"];$_f=$pg[$eh]["pos"][0]+$pg[$eh]["fields"][$Uf[1][$x]]["pos"];$ue=min($ue,$zf,$_f);$je=max($je,$zf,$_f);}echo"<div class='references' id='refl$Td' style='left: $Td"."em; top: $ue"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($je-$ue)."em;'></div></div>\n";}}}echo'</div>
<p class="links"><a href="',h(ME."schema=".urlencode($ea)),'" id="schema-link">',lang(129),'</a>
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$n){$_b="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$x)$_b.="&$x=".urlencode($_POST[$x]);cookie("adminer_export",substr($_b,1));$S=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$Bc=dump_headers((count($S)==1?key($S):DB),(DB==""||count($S)>1));$Cd=preg_match('~sql~',$_POST["format"]);if($Cd){echo"-- Adminer $ia ".$Yb[DRIVER]." dump\n\n";if($w=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
".($_POST["data_style"]?"SET foreign_key_checks = 0;
SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO';
":"")."
";$h->query("SET time_zone = '+00:00';");}}$Og=$_POST["db_style"];$l=array(DB);if(DB==""){$l=$_POST["databases"];if(is_string($l))$l=explode("\n",rtrim(str_replace("\r","",$l),"\n"));}foreach((array)$l
as$m){$b->dumpDatabase($m);if($h->select_db($m)){if($Cd&&preg_match('~CREATE~',$Og)&&($j=$h->result("SHOW CREATE DATABASE ".idf_escape($m),1))){set_utf8mb4($j);if($Og=="DROP+CREATE")echo"DROP DATABASE IF EXISTS ".idf_escape($m).";\n";echo"$j;\n";}if($Cd){if($Og)echo
use_sql($m).";\n\n";$hf="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$jg){foreach(get_rows("SHOW $jg STATUS WHERE Db = ".q($m),null,"-- ")as$K){$j=remove_definer($h->result("SHOW CREATE $jg ".idf_escape($K["Name"]),2));set_utf8mb4($j);$hf.=($Og!='DROP+CREATE'?"DROP $jg IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$j;;\n\n";}}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$K){$j=remove_definer($h->result("SHOW CREATE EVENT ".idf_escape($K["Name"]),3));set_utf8mb4($j);$hf.=($Og!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$j;;\n\n";}}if($hf)echo"DELIMITER ;;\n\n$hf"."DELIMITER ;\n\n";}if($_POST["table_style"]||$_POST["data_style"]){$ai=array();foreach(table_status('',true)as$C=>$R){$Q=(DB==""||in_array($C,(array)$_POST["tables"]));$Eb=(DB==""||in_array($C,(array)$_POST["data"]));if($Q||$Eb){if($Bc=="tar"){$rh=new
TmpFile;ob_start(array($rh,'write'),1e5);}$b->dumpTable($C,($Q?$_POST["table_style"]:""),(is_view($R)?2:0));if(is_view($R))$ai[]=$C;elseif($Eb){$p=fields($C);$b->dumpData($C,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($C));}if($Cd&&$_POST["triggers"]&&$Q&&($Ah=trigger_sql($C,$_POST["table_style"])))echo"\nDELIMITER ;;\n$Ah\nDELIMITER ;\n";if($Bc=="tar"){ob_end_flush();tar_file((DB!=""?"":"$m/")."$C.csv",$rh);}elseif($Cd)echo"\n";}}foreach($ai
as$Zh)$b->dumpTable($Zh,$_POST["table_style"],1);if($Bc=="tar")echo
...
';$Lc=true;foreach($Cf
as$x=>$X){if($x!=""&&$X>1){echo($Lc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$x%")."'>".h($x)."</a>";$Lc=false;}}}elseif(isset($_GET["privileges"])){page_header(lang(63));$I=$h->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$Wc=$I;if(!$I)$I=$h->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($Wc?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".lang(33)."<th>".lang(32)."<th>&nbsp;</thead>\n";while($K=$I->fetch_assoc())echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.lang(10)."</a>\n";if(!$Wc||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".lang(10)."'>\n";echo"</table>\n","</form>\n",'<p class="links"><a href="'.h(ME).'user=">'.lang(135)."</a>";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$gd=&get_session("queries");$fd=&$gd[DB];if(!$n&&$_POST["clear"]){$fd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?lang(64):lang(55)),$n);if(!$n&&$_POST){$Tc=false;if(!isset($_GET["import"]))$H=$_POST["query"];elseif($_POST["webfile"]){$Tc=@fopen((file_exists("adminer.sql")?"adminer.sql":"compress.zlib://adminer.sql.gz"),"rb");$H=($Tc?fread($Tc,1e6):false);}else$H=get_file("sql_file",true);if(is_string($H)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($H)+memory_get_usage()+8e6));if($H!=""&&strlen($H)<1e6){$Mf=$H.(preg_match("~;[ \t\r\n]*\$~",$H)?"":";");if(!$fd||reset(end($fd))!=$Mf){restart_session();$fd[]=array($Mf,time());set_session("queries",$gd);stop_session();}}$Gg="(?:\\s|/\\*.*\\*/|(?:#|-- )[^\n]*\n|--\r?\n)";$Ob=";";$D=0;$nc=true;$i=connect();if(is_object($i)&&DB!="")$i->select_db(DB);$qb=0;$sc=array();$Zd=0;$mf='[\'"'.($w=="sql"?'`#':($w=="sqlite"?'`[':($w=="mssql"?'[':''))).']|/\\*|-- |$'.($w=="pgsql"?'|\\$[^$]*\\$':'');$uh=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$ec=$b->dumpFormat();unset($ec["sql"]);while($H!=""){if(!$D&&preg_match("~^$Gg*DELIMITER\\s+(\\S+)~i",$H,$B)){$Ob=$B[1];$H=substr($H,strlen($B[0]));}else{preg_match('('.preg_quote($Ob)."\\s*|$mf)",$H,$B,PREG_OFFSET_CAPTURE,$D);list($Rc,$yf)=$B[0];if(!$Rc&&$Tc&&!feof($Tc))$H.=fread($Tc,1e5);else{if(!$Rc&&rtrim($H)=="")break;$D=$yf+strlen($Rc);if($Rc&&rtrim($Rc)!=$Ob){while(preg_match('('.($Rc=='/*'?'\\*/':($Rc=='['?']':(preg_match('~^-- |^#~',$Rc)?"\n":preg_quote($Rc)."|\\\\."))).'|$)s',$H,$B,PREG_OFFSET_CAPTURE,$D)){$ng=$B[0][0];if(!$ng&&$Tc&&!feof($Tc))$H.=fread($Tc,1e5);else{$D=$B[0][1]+strlen($ng);if($ng[0]!="\\")break;}}}else{$nc=false;$Mf=substr($H,0,$yf);$qb++;$Ff="<pre id='sql-$qb'><code class='jush-$w'>".shorten_utf8(trim($Mf),1000)."</code></pre>\n";if(!$_POST["only_errors"]){echo$Ff;ob_flush();flush();}$Jg=microtime(true);if($h->multi_query($Mf)&&is_object($i)&&preg_match("~^$Gg*USE\\b~isU",$Mf))$i->query($Mf);do{$I=$h->store_result();$kh=" <span class='time'>(".format_time($Jg).")</span>".(strlen($Mf)<1000?" <a href='".h(ME)."sql=".urlencode(trim($Mf))."'>".lang(10)."</a>":"");if($h->error){echo($_POST["only_errors"]?$Ff:""),"<p class='error'>".lang(136).($h->errno?" ($h->errno)":"").": ".error()."\n";$sc[]=" <a href='#sql-$qb'>$qb</a>";if($_POST["error_stops"])break
2;}elseif(is_object($I)){$z=$_POST["limit"];$bf=select($I,$i,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$Ee=$I->num_rows;echo"<p>".($Ee?($z&&$Ee>$z?lang(137,$z):"").lang(138,$Ee):""),$kh;$kd="export-$qb";$Ac=", <a href='#$kd' onclick=\"return !toggle('$kd');\">".lang(65)."</a><span id='$kd' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$ec,$xa["format"])."<input type='hidden' name='query' value='".h($Mf)."'>"." <input type='submit' name='export' value='".lang(65)."'><input type='hidden' name='token' value='$T'></span>\n";if($i&&preg_match("~^($Gg|\\()*SELECT\\b~isU",$Mf)&&($_c=explain($i,$Mf))){$kd="explain-$qb";echo", <a href='#$kd' onclick=\"return !toggle('$kd');\">EXPLAIN</a>$Ac","<div id='$kd' class='hidden'>\n";select($_c,$i,$bf);echo"</div>\n";}else
echo$Ac;echo"</form>\n";}}else{if(preg_match("~^$Gg*(CREATE|DROP|ALTER)$Gg+(DATABASE|SCHEMA)\\b~isU",$Mf)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($h->info)."'>".lang(139,$h->affected_rows)."$kh\n";}$Jg=microtime(true);}while($h->next_result());$Zd+=substr_count($Mf.$Rc,"\n");$H=substr($H,$D);$D=0;}}}}if($nc)echo"<p class='message'>".lang(140)."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(141,$qb-count($sc))," <span class='time'>(".format_time($uh).")</span>\n";}elseif($sc&&$qb>1)echo"<p class='error'>".lang(136).": ".implode("",$sc)."\n";}else
...
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),lang(149))."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),lang(150))."\n","<input type='hidden' name='token' value='$T'>\n";if(!isset($_GET["import"])&&$fd){print_fieldset("history",lang(151),$_GET["history"]!="");for($X=end($fd);$X;$X=prev($fd)){$x=key($fd);list($Mf,$kh,$ic)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.lang(10)."</a>"." <span class='time' title='".@date('Y-m-d',$kh)."'>".@date("H:i:s",$kh)."</span>"." <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$Mf)))),80,"</code>").($ic?" <span class='time'>($ic)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".lang(152)."'>\n","<a href='".h(ME."sql=&history=all")."'>".lang(153)."</a>\n","</div></fieldset>\n";}echo'</form>
';}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$p=fields($a);$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$p):""):where($_GET,$p));$Lh=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($p
as$C=>$o){if(!isset($o["privileges"][$Lh?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Lh?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$Gh=unique_array($_GET["where"],$v);$Pf="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,lang(154),$Xb->delete($a,$Pf,!$Gh));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Lh){if(!$O)redirect($A);queries_redirect($A,lang(155),$Xb->update($a,$O,$Pf,!$Gh));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$Xb->insert($a,$O);$Sd=($I?last_id():0);queries_redirect($A,lang(156,($Sd?" $Sd":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
as$C=>$o){if(isset($o["privileges"]["select"])){$Ga=convert_field($o);if($_POST["clone"]&&$o["auto_increment"])$Ga="''";if($w=="sql"&&preg_match("~enum|set~",$o["type"]))$Ga="1*".idf_escape($C);$M[]=($Ga?"$Ga AS ":"").idf_escape($C);}}$K=array();if(!support("table"))$M=array("*");if($M){$I=$Xb->select($a,$M,array($Z),$M,array(),(isset($_GET["select"])?2:1));$K=$I->fetch_assoc();if(!$K)$K=false;if(isset($_GET["select"])&&(!$K||$I->fetch_assoc()))$K=null;}}if(!support("table")&&!$p){if(!$Z){$I=$Xb->select($a,array("*"),$Z,array("*"));$K=($I?$I->fetch_assoc():false);if(!$K)$K=array($Xb->primary=>"");}if($K){foreach($K
as$x=>$X){if(!$Z)$K[$x]=null;$p[$x]=array("field"=>$x,"null"=>($x!=$Xb->primary),"auto_increment"=>($x==$Xb->primary));}}}edit_form($a,$p,$K,$Lh);}elseif(isset($_GET["create"])){$a=$_GET["create"];$nf=array();foreach(array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST')as$x)$nf[$x]=$x;$Wf=referencable_primary($a);$Pc=array();foreach($Wf
as$Vg=>$o)$Pc[str_replace("`","``",$Vg)."`".str_replace("`","``",$o["field"])]=$Vg;$ef=array();$R=array();if($a!=""){$ef=fields($a);$R=table_status($a);if(!$R)$n=lang(9);}$K=$_POST;$K["fields"]=(array)$K["fields"];if($K["auto_increment_col"])$K["fields"][$K["auto_increment_col"]]["auto_increment"]=true;if($_POST&&!process_fields($K["fields"])&&!$n){if($_POST["drop"])queries_redirect(substr(ME,0,-1),lang(157),drop_tables(array($a)));else{$p=array();$Da=array();$Ph=false;$Nc=array();ksort($K["fields"]);$df=reset($ef);$Aa=" FIRST";foreach($K["fields"]as$x=>$o){$q=$Pc[$o["type"]];$Bh=($q!==null?$Wf[$q]:$o);if($o["field"]!=""){if(!$o["has_default"])$o["default"]=null;if($x==$K["auto_increment_col"])$o["auto_increment"]=true;$Kf=process_field($o,$Bh);$Da[]=array($o["orig"],$Kf,$Aa);if($Kf!=process_field($df,$df)){$p[]=array($o["orig"],$Kf,$Aa);if($o["orig"]!=""||$Aa)$Ph=true;}if($q!==null)$Nc[idf_escape($o["field"])]=($a!=""&&$w!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$Pc[$o["type"]],'source'=>array($o["field"]),'target'=>array($Bh["field"]),'on_delete'=>$o["on_delete"],));$Aa=" AFTER ".idf_escape($o["field"]);}elseif($o["orig"]!=""){$Ph=true;$p[]=array($o["orig"]);}if($o["orig"]!=""){$df=next($ef);if(!$df)$Aa="";}}$pf="";if($nf[$K["partition_by"]]){$qf=array();if($K["partition_by"]=='RANGE'||$K["partition_by"]=='LIST'){foreach(array_filter($K["partition_names"])as$x=>$X){$Y=$K["partition_values"][$x];$qf[]="\n  PARTITION ".idf_escape($X)." VALUES ".($K["partition_by"]=='RANGE'?"LESS THAN":"IN").($Y!=""?" ($Y)":" MAXVALUE");}}$pf.="\nPARTITION BY $K[partition_by]($K[partition])".($qf?" (".implode(",",$qf)."\n)":($K["partitions"]?" PARTITIONS ".(+$K["partitions"]):""));}elseif(support("partitioning")&&preg_match("~partitioned~",$R["Create_options"]))$pf.="\nREMOVE PARTITIONING";$ne=lang(158);if($a==""){cookie("adminer_engine",$K["Engine"]);$ne=lang(159);}$C=trim($K["name"]);queries_redirect(ME.(support("table")?"table=":"select=").urlencode($C),$ne,alter_table($a,$C,($w=="sqlite"&&($Ph||$Nc)?$Da:$p),$Nc,($K["Comment"]!=$R["Comment"]?$K["Comment"]:null),($K["Engine"]&&$K["Engine"]!=$R["Engine"]?$K["Engine"]:""),($K["Collation"]&&$K["Collation"]!=$R["Collation"]?$K["Collation"]:""),($K["Auto_increment"]!=""?number($K["Auto_increment"]):""),$pf));}}page_header(($a!=""?lang(41):lang(66)),$n,array("table"=>$a),h($a));if(!$_POST){$K=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($Dh["int"])?"int":(isset($Dh["integer"])?"integer":"")))),"partition_names"=>array(""),);if($a!=""){$K=$R;$K["name"]=$a;$K["fields"]=array();if(!$_GET["auto_increment"])$K["Auto_increment"]="";foreach($ef
as$o){$o["has_default"]=isset($o["default"]);$K["fields"][]=$o;}if(support("partitioning")){$Uc="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($a);$I=$h->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $Uc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($K["partition_by"],$K["partitions"],$K["partition"])=$I->fetch_row();$qf=get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Uc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");$qf[""]="";$K["partition_names"]=array_keys($qf);$K["partition_values"]=array_values($qf);}}}$nb=collations();$pc=engines();foreach($pc
...
';}elseif(isset($_GET["indexes"])){$a=$_GET["indexes"];$qd=array("PRIMARY","UNIQUE","INDEX");$R=table_status($a,true);if(preg_match('~MyISAM|M?aria'.($h->server_info>=5.6?'|InnoDB':'').'~i',$R["Engine"]))$qd[]="FULLTEXT";$v=indexes($a);$Df=array();if($w=="mongo"){$Df=$v["_id_"];unset($qd[0]);unset($v["_id_"]);}$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]){$c=array();foreach($K["indexes"]as$u){$C=$u["name"];if(in_array($u["type"],$qd)){$f=array();$Xd=array();$Qb=array();$O=array();ksort($u["columns"]);foreach($u["columns"]as$x=>$e){if($e!=""){$y=$u["lengths"][$x];$Pb=$u["descs"][$x];$O[]=idf_escape($e).($y?"(".(+$y).")":"").($Pb?" DESC":"");$f[]=$e;$Xd[]=($y?$y:null);$Qb[]=$Pb;}}if($f){$yc=$v[$C];if($yc){ksort($yc["columns"]);ksort($yc["lengths"]);ksort($yc["descs"]);if($u["type"]==$yc["type"]&&array_values($yc["columns"])===$f&&(!$yc["lengths"]||array_values($yc["lengths"])===$Xd)&&array_values($yc["descs"])===$Qb){unset($v[$C]);continue;}}$c[]=array($u["type"],$C,$O);}}}foreach($v
...
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.2.3.1' alt='+' title='<?php echo
lang(97),'\'></noscript>&nbsp;
</thead>
';if($Df){echo"<tr><td>PRIMARY<td>";foreach($Df["columns"]as$x=>$e){echo
select_input(" disabled",$p,$e),"<label><input disabled type='checkbox'>".lang(50)."</label> ";}echo"<td><td>\n";}$Gd=1;foreach($K["indexes"]as$u){if(!$_POST["drop_col"]||$Gd!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$Gd][type]",array(-1=>"")+$qd,$u["type"],($Gd==count($K["indexes"])?"indexesAddRow(this);":1)),"<td>";ksort($u["columns"]);$s=1;foreach($u["columns"]as$x=>$e){echo"<span>".select_input(" name='indexes[$Gd][columns][$s]' onchange=\"".($s==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".h(js_escape($w=="sql"?"":$_GET["indexes"]."_"))."');\"",($p?array_combine($p,$p):$p),$e),($w=="sql"||$w=="mssql"?"<input type='number' name='indexes[$Gd][lengths][$s]' class='size' value='".h($u["lengths"][$x])."'>":""),($w!="sql"?checkbox("indexes[$Gd][descs][$s]",1,$u["descs"][$x],lang(50)):"")," </span>";$s++;}echo"<td><input name='indexes[$Gd][name]' value='".h($u["name"])."' autocapitalize='off'>\n","<td><input type='image' class='icon' name='drop_col[$Gd]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.2.3.1' alt='x' title='".lang(100)."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$Gd++;}echo'</table>
<p>
<input type="submit" value="',lang(14),'">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$n&&!isset($_POST["add_x"])){$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),lang(171),drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),lang(172),rename_database($C,$K["collation"]));}else{$l=explode("\n",str_replace("\r","",$C));$Pg=true;$Rd="";foreach($l
as$m){if(count($l)==1||$m!=""){if(!create_database($m,$K["collation"]))$Pg=false;$Rd=$m;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($Rd),lang(173),$Pg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(174));}}page_header(DB!=""?lang(59):lang(175),$n,array(),h(DB));$nb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$nb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Wc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Wc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
lang(14),'">
';if(DB!="")echo"<input type='submit' name='drop' value='".lang(116)."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.3.1' alt='+' title='".lang(97)."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$C=$_GET["name"];$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$ne=($_POST["drop"]?lang(181):($C!=""?lang(182):lang(183)));$A=ME."table=".urlencode($a);$K["source"]=array_filter($K["source"],'strlen');ksort($K["source"]);$dh=array();foreach($K["source"]as$x=>$X)$dh[$x]=$K["target"][$x];$K["target"]=$dh;if($w=="sqlite")queries_redirect($A,$ne,recreate_table($a,$a,array(),array(),array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($K)))));else{$c="ALTER TABLE ".table($a);$Zb="\nDROP ".($w=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);if($_POST["drop"])query_redirect($c.$Zb,$A,$ne);else{query_redirect($c.($C!=""?"$Zb,":"")."\nADD".format_foreign_key($K),$A,$ne);$n=lang(184)."<br>$n";}}}page_header(lang(185),$n,array("table"=>$a),h($a));if($_POST){ksort($K["source"]);if($_POST["add"])$K["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$K["target"]=array();}elseif($C!=""){$Pc=foreign_keys($a);$K=$Pc[$C];$K["source"][]="";}else{$K["table"]=$a;$K["source"]=array("");}$Fg=array_keys(fields($a));$dh=($a===$K["table"]?$Fg:array_keys(fields($K["table"])));$Vf=array_keys(array_filter(table_status('',true),'fk_support'));echo'
<form action="" method="post">
<p>
';if($K["db"]==""&&$K["ns"]==""){echo
lang(186),':
...
';$Gd=0;foreach($K["source"]as$x=>$X){echo"<tr>","<td>".html_select("source[".(+$x)."]",array(-1=>"")+$Fg,$X,($Gd==count($K["source"])-1?"foreignAddRow(this);":1)),"<td>".html_select("target[".(+$x)."]",$dh,$K["target"][$x]);$Gd++;}echo'</table>
<p>
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$n){$C=trim($K["name"]);$Ga=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ne=lang(189);if($_GET["materialized"])$U="MATERIALIZED VIEW";else{$U="VIEW";if($w=="pgsql"){$Kg=table_status($C);$U=($Kg?strtoupper($Kg["Engine"]):$U);}}if(!$_POST["drop"]&&$a==$C&&$w!="sqlite"&&$U!="MATERIALIZED VIEW")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$ne);else{$fh=$C."_adminer_".uniqid();drop_create("DROP $U ".table($a),"CREATE $U ".table($C).$Ga,"DROP $U ".table($C),"CREATE $U ".table($fh).$Ga,"DROP $U ".table($fh),($_POST["drop"]?substr(ME,0,-1):$A),lang(190),$ne,lang(191),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$n)$n=error();}page_header(($a!=""?lang(40):lang(192)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>',lang(200),'<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$yd,$K["INTERVAL_FIELD"]),'<tr><th>',lang(107),'<td>',html_select("STATUS",$Lg,$K["STATUS"]),'<tr><th>',lang(96),'<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$jg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$n){$fh="$K[name]_adminer_".uniqid();drop_create("DROP $jg ".idf_escape($da),create_routine($jg,$K),"DROP $jg ".idf_escape($K["name"]),create_routine($jg,array("name"=>$fh)+$K),"DROP $jg ".idf_escape($fh),substr(ME,0,-1),lang(202),lang(203),lang(204),$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?lang(205):lang(206)).": ".h($da):(isset($_GET["function"])?lang(207):lang(208))),$n);if(!$_POST&&$da!=""){$K=routine($da,$jg);$K["name"]=$da;}$nb=get_vals("SHOW CHARACTER SET");sort($nb);$kg=routine_languages();echo'
<form action="" method="post" id="form">
<p>',lang(170),': <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($kg?lang(19).": ".html_select("language",$kg,$K["language"]):""),'<input type="submit" value="',lang(14),'">
<table cellspacing="0" class="nowrap">
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$_h=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$_h["Timing"])&&in_array($_POST["Event"],$_h["Event"])&&in_array($_POST["Type"],$_h["Type"])){$Ne=" ON ".table($a);$Zb="DROP TRIGGER ".idf_escape($C).($w=="pgsql"?$Ne:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Zb,$A,lang(219));else{if($C!="")queries($Zb);queries_redirect($A,($C!=""?lang(220):lang(221)),queries(create_trigger($Ne,$_POST)));if($C!="")queries(create_trigger($Ne,$K+array("Type"=>reset($_h["Type"]))));}}$K=$_POST;}page_header(($C!=""?lang(222).": ".h($C):lang(223)),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>',lang(224),'<td>',html_select("Timing",$_h["Timing"],$K["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>',lang(225),'<td>',html_select("Event",$_h["Event"],$K["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$_h["Event"])?" <input name='Of' value='".h($K["Of"])."' class='hidden'>":""),'<tr><th>',lang(92),'<td>',html_select("Type",$_h["Type"],$K["Type"]),'</table>
...
as$Ge=>$Wc){echo'<th>'.($Ge!="*.*"?"<input name='objects[$s]' value='".h($Ge)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(119),"Columns"=>lang(120),"Procedures"=>lang(230),)as$yb=>$Pb){foreach((array)$If[$yb]as$Hf=>$rb){echo"<tr".odd()."><td".($Pb?">$Pb<td":" colspan='2'").' lang="en" title="'.h($rb).'">'.h($Hf);$s=0;foreach($Xc
...
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$R=table_status1($a);$v=indexes($a);$p=fields($a);$Pc=column_foreign_keys($a);$Ie="";if($R["Oid"]){$Ie=($w=="sqlite"?"rowid":"oid");$v[]=array("type"=>"PRIMARY","columns"=>array($Ie));}parse_str($_COOKIE["adminer_import"],$ya);$hg=array();$f=array();$jh=null;foreach($p
as$x=>$o){$C=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$C!=""){$f[$x]=html_entity_decode(strip_tags($C),ENT_QUOTES);if(is_shortable($o))$jh=$b->selectLengthProcess();}$hg+=$o["privileges"];}list($M,$Yc)=$b->selectColumnsProcess($f,$v);$Bd=count($Yc)<count($M);$Z=$b->selectSearchProcess($p,$v);$Ye=$b->selectOrderProcess($p,$v);$z=$b->selectLimitProcess();$Uc=($M?implode(", ",$M):"*".($Ie?", $Ie":"")).convert_fields($f,$p,$M)."\nFROM ".table($a);$Zc=($Yc&&$Bd?"\nGROUP BY ".implode(", ",$Yc):"").($Ye?"\nORDER BY ".implode(", ",$Ye):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Hh=>$K){$Ga=convert_field($p[key($K)]);$M=array($Ga?$Ga:idf_escape(key($K)));$Z[]=where_check($Hh,$p);$J=$Xb->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}if($_POST&&!$n){$ei=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$eb=array();foreach($_POST["check"]as$bb)$eb[]=where_check($bb,$p);$ei[]="((".implode(") OR (",$eb)."))";}$ei=($ei?"\nWHERE ".implode(" AND ",$ei):"");$Df=$Jh=null;foreach($v
as$u){if($u["type"]=="PRIMARY"){$Df=array_flip($u["columns"]);$Jh=($M?$Df:array());break;}}foreach((array)$Jh
as$x=>$X){if(in_array(idf_escape($x),$M))unset($Jh[$x]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Jh===array())$H="SELECT $Uc$ei$Zc";else{$Fh=array();foreach($_POST["check"]as$X)$Fh[]="(SELECT".limit($Uc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$Zc,1).")";$H=implode(" UNION ALL ",$Fh);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Pc)){if($_POST["save"]||$_POST["delete"]){$I=true;$za=0;$O=array();if(!$_POST["delete"]){foreach($f
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($Jh===array()&&is_array($_POST["check"]))||$Bd){$I=($_POST["delete"]?$Xb->delete($a,$ei):($_POST["clone"]?queries("INSERT $H$ei"):$Xb->update($a,$O,$ei)));$za=$h->affected_rows;}else{foreach((array)$_POST["check"]as$X){$di="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$Xb->delete($a,$di,1):($_POST["clone"]?queries("INSERT".limit1($H,$di)):$Xb->update($a,$O,$di)));if(!$I)break;$za+=$h->affected_rows;}}}$ne=lang(237,$za);if($_POST["clone"]&&$I&&$za==1){$Sd=last_id();if($Sd)$ne=lang(156," $Sd");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$ne,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(238);else{$I=true;$za=0;foreach($_POST["val"]as$Hh=>$K){$O=array();foreach($K
as$x=>$X){$x=bracket_escape($x,1);$O[idf_escape($x)]=(preg_match('~char|text~',$p[$x]["type"])||$X!=""?$b->processInput($p[$x],$X):"NULL");}$I=$Xb->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Hh,$p),!($Bd||$Jh===array())," ");if(!$I)break;$za+=$h->affected_rows;}queries_redirect(remove_from_uri(),lang(237,$za),$I);}}elseif(!is_string($Ic=get_file("csv_file",true)))$n=upload_error($Ic);elseif(!preg_match('~~u',$Ic))$n=lang(239);else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$I=true;$ob=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Ic,$fe);$za=count($fe[0]);$Xb->begin();$wg=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($fe[0]as$x=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$wg]*)$wg~",$X.$wg,$ge);if(!$x&&!array_diff($ge[1],$ob)){$ob=$ge[1];$za--;}else{$O=array();foreach($ge[1]as$s=>$lb)$O[idf_escape($ob[$s])]=($lb==""&&$p[$ob[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$lb))));$L[]=$O;}}$I=(!$L||$Xb->insertUpdate($a,$L,$Df));if($I)$Xb->commit();queries_redirect(remove_from_uri("page"),lang(240,$za),$I);$Xb->rollback();}}}$Vg=$b->tableName($R);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(44).": $Vg",$n);$O=null;if(isset($hg["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Pc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($R,$O);if(!$f&&support("table"))echo"<p class='error'>".lang(241).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($Z,$f,$v);$b->selectOrderPrint($Ye,$f,$v);$b->selectLimitPrint($z);$b->selectLengthPrint($jh);$b->selectActionPrint($v);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Sc=$h->result(count_rows($a,$Z,$Bd,$Yc));$E=floor(max(0,$Sc-1)/$z);}$tg=$M;if(!$tg){$tg[]="*";if($Ie)$tg[]=$Ie;}$zb=convert_fields($f,$p,$M);if($zb)$tg[]=substr($zb,2);$I=$Xb->select($a,$tg,$Z,$Yc,$Ye,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($w=="mssql"&&$E)$I->seek($z*$E);$mc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$w=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$Yc&&$Bd&&$w=="sql")$Sc=$h->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$Pa=$b->backwardKeys($a,$Vg);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Yc&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(242)."</a>");$ze=array();$Vc=array();reset($M);$Rf=1;foreach($L[0]as$x=>$X){if($x!=$Ie){$X=$_GET["columns"][key($M)];$o=$p[$M?($X?$X["col"]:current($M)):$x];$C=($o?$b->fieldName($o,$Rf):($X["fun"]?"*":$x));if($C!=""){$Rf++;$ze[$x]=$C;$e=idf_escape($x);$jd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);$Pb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($jd.($Ye[0]==$e||$Ye[0]==$x||(!$Ye&&$Bd&&$Yc[0]==$e)?$Pb:'')).'">';echo
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($jd.$Pb)."' title='".lang(50)."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.lang(47).'" class="text jsonly"> =</a>';echo"</span>";}$Vc[$x]=$X["fun"];next($M);}}$Xd=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$x=>$X)$Xd[$x]=max($Xd[$x],min(40,strlen(utf8_decode($X))));}}echo($Pa?"<th>".lang(243):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Pc)as$ye=>$K){$Gh=unique_array($L[$ye],$v);if(!$Gh){$Gh=array();foreach($L[$ye]as$x=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$x))$Gh[$x]=$X;}}$Hh="";foreach($Gh
as$x=>$X){if(($w=="sql"||$w=="pgsql")&&strlen($X)>64){$x=(strpos($x,'(')?$x:idf_escape($x));$x="MD5(".($w=='sql'&&preg_match("~^utf8_~",$p[$x]["collation"])?$x:"CONVERT($x USING ".charset($h).")").")";$X=md5($X);}$Hh.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X):"null%5B%5D=".urlencode($x));}echo"<tr".odd().">".(!$Yc&&$M?"":"<td>".checkbox("check[]",substr($Hh,1),in_array(substr($Hh,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Bd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Hh)."'>".lang(244)."</a>"));foreach($K
as$x=>$X){if(isset($ze[$x])){$o=$p[$x];if($X!=""&&(!isset($mc[$x])||$mc[$x]!=""))$mc[$x]=(is_mail($X)?$ze[$x]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$o["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($x).$Hh;if(!$_&&$X!==null){foreach((array)$Pc[$x]as$q){if(count($Pc[$x])==1||end($q["source"])==$x){$_="";foreach($q["source"]as$s=>$Fg)$_.=where_link($s,$q["target"][$s],$L[$ye][$Fg]);$_=($q["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($q["db"]),ME):ME).'select='.urlencode($q["table"]).$_;if(count($q["source"])==1)break;}}}if($x=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$Gh))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($Gh
as$Hd=>$W)$_.=where_link($s++,$Hd,$W);}$X=select_value($X,$_,$o,$jh);$kd=h("val[$Hh][".bracket_escape($x)."]");$Y=$_POST["val"][$Hh][bracket_escape($x)];$hc=!is_array($K[$x])&&is_utf8($X)&&$L[$ye][$x]==$K[$x]&&!$Vc[$x];$ih=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$hc)||$Y!==null){$bd=h($Y!==null?$Y:$K[$x]);echo"<td>".($ih?"<textarea name='$kd' cols='30' rows='".(substr_count($K[$x],"\n")+1)."'>$bd</textarea>":"<input name='$kd' value='$bd' size='$Xd[$x]'>");}else{$ce=strpos($X,"<i>...</i>");echo"<td id='$kd' onclick=\"selectClick(this, event, ".($ce?2:($ih?1:0)).($hc?"":", '".h(lang(245))."'").");\">$X";}}}if($Pa)echo"<td>";$b->backwardKeysPrint($Pa,$L[$ye]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$wc=true;if($_GET["page"]!="last"){if(!+$z)$Sc=count($L);elseif($w!="sql"||!$Bd){$Sc=($Bd?false:found_rows($R,$Z));if($Sc<max(1e4,2*($E+1)*$z))$Sc=reset(slow_query(count_rows($a,$Z,$Bd,$Yc)));else$wc=false;}}if(+$z&&($Sc===false||$Sc>$z||$E)){echo"<p class='pages'>";$ie=($Sc===false?$E+(count($L)>=$z?2:1):floor(($Sc-1)/$z));if($w!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(246)."', '".($E+1)."'), event); return false;\">".lang(246)."</a>:",pagination(0,$E).($E>5?" ...":"");for($s=max(1,$E-4);$s<min($ie,$E+5);$s++)echo
